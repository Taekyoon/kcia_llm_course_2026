...
for item in "${TARGETS[@]}"; do
  ...
  # RAG개선은 rag 실습이 만든 인덱스가 있어야 돈다. 서버 없는 slow 그룹에서
  # 인덱스가 없으면 "실패"가 아니라 "건너뜀"이다 (2026-08-27 실측 — 6초 실패로 집계됐었다).
  case "$name" in *RAG개선*)
    if [ ! -d "$(dirname "$W/$rel")/bm25_retriever" ]; then
      echo "   ⏭ 건너뜀 — bm25_retriever 없음 (rag 실습 미실행. 서버 검증 때 함께 확인)"
      SKIP+=("$name"); continue
    fi ;;
  esac
...
echo " 결과"
echo "======================================================================"
[ "${#SKIP[@]}" -gt 0 ] && echo " 건너뜀 ${#SKIP[@]}종: ${SKIP[*]}"
