...
**배치를 쪼개는 것은 메모리 때문입니다.** 한 번에 16개를 올리면 GPU 가 터집니다.
...
지금까지는 **모델을 고치는** 방법을 봤습니다. SFT · DPO · GRPO 전부 가중치를 바꿉니다.

그런데 문제가 **"모델이 모른다"** 일 때는 학습이 답이 아닐 때가 많습니다.
...
vLLM 서버에 HTTP 로 붙습니다. 3일차 다른 실습과 같은 서버입니다.
